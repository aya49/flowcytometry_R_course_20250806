...
# change where you want to save cleaning plots!
res_dir <- paste0(home_dir, "/res")
dir.create(res_dir)

# path to fcs file
fcs_path <- system.file("extdata", "111.fcs", package="PeacoQC")
# fcs_path <- paste0(home_dir, "/sangerP2.fcs")
...
# plot your flow frame against time
channel <- "FSC-A"
flowDensity::plotDens(f, channels=c("Time", channel))


## remove events with extreme values ####

# set an upper threshold for FSC-A values
# i.e. events with FSC-A above this is removed.
gate_margin <- 150000
...
# before compensation + after transformation plot
flowDensity::plotDens(f1t, channels=c("G780-A", "G710-A"))

# after compensation + after transformation plot
flowDensity::plotDens(f, channels=c("G780-A", "G710-A"))


## 1.3 cleaning: peacoqc; see res_dir for plot ####
...
colnames(flowCore::exprs(f))
flowDensity::plotDens(fc, channels=c("Time", "B515-A"), xlim=time_range)

# save file
fcs_path_clean = gsub("[.]fcs", "_clean.fcs", fcs_path)
flowCore::write.fcs(fc, filename=fcs_path_clean)
...
## 1.3 cleaning: flowcut; see res_dir for plot ####
flCt <- flowCut::flowCut(f, Plot="All", Directory=res_dir)
...
colnames(flowCore::exprs(f))
flowDensity::plotDens(fct, channels=c("Time", "B515-A"), xlim=time_range)
