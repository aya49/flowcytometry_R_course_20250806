...
t2 <- Rtsne::Rtsne(ffs)$Y # tsne
u2 <- umap::umap(ffs)$layout # umap
pca_res <- prcomp(ffs) # pca
# TRY CHANGING columns used c(1,2), c(2,3), ... (PC's used)
p2 <- pca_res[,c(1:2)]
...
colnames(t2) <- colnames(u2) <- colnames(p2) <- c("x", "y")


## plot clusters and ground truth cell populations in 2D
tup2d <- data.frame(
    tx=t2[,1], ty=t2[,2], ux=u2[,1], uy=u2[,2], ux=p2[,1], uy=p2[,2],
    phenograph=pc[ffsample], flowsom=fc[ffsample],
    cpops=sapply(apply(cpops_matrix[ffsample,], 1, function(x)
        (cpops[x])), function(y) y[1]) )

# tsne
gptp <- ggplot2::ggplot(tup2d, ggplot2::aes(x=tx, y=ty, colour=factor(phenograph))) + ggplot2::geom_point(size=0.5)
gptf <- ggplot2::ggplot(tup2d, ggplot2::aes(x=tx, y=ty, colour=factor(flowsom))) + ggplot2::geom_point(size=0.5)
gptc <- ggplot2::ggplot(tup2d, ggplot2::aes(x=tx, y=ty, colour=factor(cpops))) + ggplot2::geom_point(size=0.5)

# umap
gpup <- ggplot2::ggplot(tup2d, ggplot2::aes(x=ux, y=uy, colour=factor(phenograph))) + ggplot2::geom_point(size=0.5)
gpuf <- ggplot2::ggplot(tup2d, ggplot2::aes(x=ux, y=uy, colour=factor(flowsom))) + ggplot2::geom_point(size=0.5)
gpuc <- ggplot2::ggplot(tup2d, ggplot2::aes(x=ux, y=uy, colour=factor(cpops))) + ggplot2::geom_point(size=0.5)

# umap
gppp <- ggplot2::ggplot(tup2d, ggplot2::aes(x=px, y=py, colour=factor(phenograph))) + ggplot2::geom_point(size=0.5)
gppf <- ggplot2::ggplot(tup2d, ggplot2::aes(x=px, y=py, colour=factor(flowsom))) + ggplot2::geom_point(size=0.5)
gppc <- ggplot2::ggplot(tup2d, ggplot2::aes(x=px, y=py, colour=factor(cpops))) + ggplot2::geom_point(size=0.5)
